...
`/sub/contents/{contentId}/chat`을 SUBSCRIBE하면, 브로커 등록 직후 그 세션에만
`ContentChatDto` 프레임이 오래된 순서로 최대 `chat-buffer-size`건 도착합니다.
페이로드 형식은 실시간 메시지와 동일하며, 같은 목적지를 이미 구독 중인 다른 세션은
받지 않습니다. 버퍼가 비어 있거나 조회가 실패하면 아무것도 오지 않고 구독 자체는
정상 유지됩니다.

`chat-buffer-ttl` 동안 새 메시지가 없으면 버퍼가 비워지므로, 그 이후 구독에는
백로그가 오지 않습니다.

**중복·순서 역전 가능**: 브로커 구독 등록과 백로그 조회 사이에 다른 사용자가
메시지를 보내면, 신규 구독자가 그 메시지를 백로그와 라이브 양쪽으로 두 번 받거나
라이브가 백로그보다 먼저 도착할 수 있습니다. `ContentChatDto`에 식별자가 없어
현재는 클라이언트가 중복 제거·재정렬을 할 수 없습니다.

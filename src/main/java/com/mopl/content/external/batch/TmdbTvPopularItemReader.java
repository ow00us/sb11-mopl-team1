 ...
 * 특정 페이지 조회가 실패하면 로그만 남기고 다음 페이지로 넘어간다 — 페이지 커서를
 * API 호출 전에 미리 전진시켜서, 같은 페이지를 반복 요청하다 skipLimit을 소진하는 것을 방지한다.
...
public class TmdbTvPopularItemReader implements ItemReader<TmdbTvSummary> {
    ...
    public TmdbTvSummary read() {
        while (!currentPageIterator.hasNext()) {
            ...
            TmdbPopularTvResponse response;
            try {
                response = tmdbApiClient.getPopularTvShows(pageToFetch);
            } catch (TmdbApiException e) {
                log.warn("TMDB 인기 TV {}페이지 조회 실패, 다음 페이지로 넘어갑니다.", pageToFetch, e);
                continue;
            }
